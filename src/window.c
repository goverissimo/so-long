...
/*   Updated: 2023/12/01 21:20:40 by gverissi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_images(void)
{
	...
	i = PIXELS;
	(window()->p) = mlx_xpm_file_to_image(window()->mlx_ptr, "./sprites/player.xpm", &i, &i);
...
void	load_images(int i, int j)
{
	if(game()->map[i][j] == '1')
		...
		mlx_put_image_to_window(window()->mlx_ptr, window()->win_ptr, window()->p, j* 64, i* 64);
